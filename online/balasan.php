...
<div class="w-7/12 h-fit my-12 p-4 flex justify-center bg-neutral-100 rounded-3xl shadow-xl">
	...
	<form action="../controller/add-proc.php" method="post" accept-charset="utf-8" class="w-full bg-base-100 rounded-2xl p-2 shadow-md">
		  ...
		  <?php 
		  	if (isset($_SESSION['role'])) {
		  		if ($_SESSION['role'] == 'admin') {
		  			...
		  			echo '
					  <textarea pattern="[a-zA-Z]+" title="Hanya huruf besar/kecil yang diperbolehkan" name="pesan" maxlength="300" class="textarea textarea-bordered resize-none" readonly>' . $row['balasan'] . '</textarea>
		  			';
		  		}
		  	}
		  ?>
		...
		<div class="flex gap-2 mt-4 justify-end">
			<a href="?page=konsultasi" class="btn btn-error text-white hover:outline hover:outline-2 hover:outline-offset-2" value="kembali" name="kembali">Kembali</a>
		</div>
	</form>
</div>